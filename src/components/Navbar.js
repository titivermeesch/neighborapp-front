...
export default class SidebarExampleTransitions extends Component {
    ...
    handleAnimationChange = () => {
        ...
        if (this.state.visible) {
            ...
            document.getElementsByClassName('fader')[0].style.visibility =
                'hidden'
        }
    }
    ...
    destroySession = () => {
        fetch(
            `http://localhost:3000/sessions/${localStorage.getItem('token')}`,
            {
                method: 'DELETE',
                headers: {
    ...
    fetchRequestLocations = () => {
        fetch(`http://localhost:3000/requests/`, {
            ...
            .then(data => {
                this.setState({
                    contributions: data.data.filter(w => w.status === 'open')
                        .length
                })
                setTimeout(() => {
